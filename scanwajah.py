    ...
    for folder_name in os.listdir(main_path):
        ...
        if os.path.isdir(folder_path):
            ...
            for image_name in os.listdir(folder_path):
                ...
                # Terapkan histogram equalization untuk meningkatkan kontras gambar
                equalized = cv2.equalizeHist(gray)

                # Deteksi wajah
                faces_detected = face_cascade.detectMultiScale(equalized, scaleFactor=1.1, minNeighbors=7, minSize=(30, 30))

                for (x, y, w, h) in faces_detected:
                    # Resize wajah yang terdeteksi dan tambahkan ke list
                    face_resized = cv2.resize(equalized[y:y+h, x:x+w], (150, 150))
...
# Path dataset
dataset_path = r'D:\Rey Rizky Andifa\S1 Teknik Informatika Rey\Mata Kuiah SMT 3\Kecerdasan Buatan\UAS\dataset_webcam'
...
if len(faces) > 0:
    ...
    # Normalisasi piksel (agar berada dalam rentang [0, 1])
    faces_normalized = [face.flatten() / 255.0 for face in faces]
    ...
    clf.fit(faces_normalized, labels_encoded)

    # Simpan model pelatihan
    joblib.dump(clf, 'svm_model.pkl')  # Menggunakan joblib untuk menyimpan model
    joblib.dump(le, 'label_encoder.pkl')  # Menyimpan label encoder dengan joblib
...
clf = joblib.load('svm_model.pkl')  # Menggunakan joblib untuk memuat model
le = joblib.load('label_encoder.pkl')  # Memuat label encoder
...
while True:
    ...
    for (x, y, w, h) in faces_detected:
        ...
        face_resized = cv2.resize(equalized[y:y+h, x:x+w], (150, 150)).flatten().reshape(1, -1)

        # Normalisasi piksel (untuk konsistensi dengan data pelatihan)
        face_resized_normalized = face_resized / 255.0

        # Prediksi identitas wajah
        label_encoded = clf.predict(face_resized_normalized)

        # Mendapatkan probabilitas dari model SVM
        proba = clf.predict_proba(face_resized_normalized)
